...
const totalSum = (input) => {
  // Solution code here...
 return input.reduce((acc,value) =>acc + value.reduce((acc1,val) => acc1+val,0),0);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  return input.map(item => item.filter(val =>(typeof val === 'number' && val %5 ===0 )).map(item2 => Math.pow(2,item2)));
};
...
let findShortest = (data) => {
  // Solution code here...
  return data.reduce((item,val)=> Number(item.height) < Number(val.height) ? item:val).name;
};
